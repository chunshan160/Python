...
class BuyGoods(unittest.TestCase):
    ...
    def setUp(cls):
        time.sleep(3)
        # 滚动至元素【现金区】可见，点击【更多】
        ActionChains(cls.driver).move_to_element(cls.driver.find_element(*Home.cash_goods_title)).perform()
        cls.driver.execute_script('window.scrollBy(0,500)')
        cls.driver.find_element(*Home.cash_goods_more).click()
        time.sleep(2)
    ...
    # 购买现金商品
    def test_1_CashRegion(self):
        # 点击【现金商品】
        self.driver.find_element(*Home.cash_goods_goods).click()
        time.sleep(6)
        # 购买流程
        # 立即购买
