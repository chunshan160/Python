...
from web_test2.PageObjects.BOSS.Login_page import LoginPage
from web_test2.PageObjects.BOSS import PublishGoods_page as PG
from web_test2.TestData.BOSS import Login_Data as LD
from web_test2.PageObjects.BOSS.SystemPoint_page import SystemPoint
from web_test2.PageLocators.BOSS.PublishGoods import Pubilc
from web_test2.TestData.BOSS import Publish_Data as PD
import unittest
...
class PiblishGoods(unittest.TestCase):
    ...
    def test_1_RealGoods_submit(cls, data):
        # 点击发布实物商品
        cls.pp.Real_Goods_cash(data["product_title"], data["limit_quantity"], data["product_description"],
    ...
    def test_3_RealGoods_submit(cls, data):
        # 点击发布实物商品
        cls.pp.Local_Life_cash(data["product_title"], data["total_price"], data["stock"],
    ...
    # 发布本地生活，放入仓库
    @ddt.data(*PD.Local_Life_data)
    def test_4_RealGoods_submit(cls, data):
        # 点击发布实物商品
        cls.pp.Local_Life_cash(data["product_title"], data["total_price"], data["stock"],
                          data["limit_quantity"], data["product_description"])
        # 放入仓库
    ...
    def test_5_RealGoods_submit(cls, data):
        # 点击发布实物商品
        cls.pp.Business_Services_cash(data["product_title"], data["total_price"], data["subsist"], data["stock"],
                                 data["limit_quantity"], data["product_description"])
        # 立即发布
        cls.pp.submit(Pubilc.submit)
        # 断言 在【发布商品】中，存在[提交审核成功] 这个元素   对比一样就ok，不一样就报错
    ...
    # 发布商企服务，放入仓库
    @ddt.data(*PD.Business_Services_data)
    def test_6_RealGoods_submit(cls, data):
        # 点击发布实物商品
        cls.pp.Business_Services_cash(data["product_title"], data["total_price"], data["subsist"], data["stock"],
                                 data["limit_quantity"], data["product_description"])
        # 放入仓库
