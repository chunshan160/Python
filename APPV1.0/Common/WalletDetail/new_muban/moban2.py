...
from Common.WalletDetail.new_muban.qushe import qushe
from Common.WalletDetail.new_muban.chuli import chulidata
from Common.WalletDetail.new_muban.calculate_commission import calculate_commission
    ...
    def moban(self, data, superior, ip, calculation, second_payagent_ratio,
        ...
        if (self.buyer_identity == "公海用户" and self.seller_identity == "个人焕商") or (
            ...
            if bind_buyer_relationship_data != None and reserve_fund_second_payagent_ratio != None:
                ...
                if self.payment_method in ["易贝", "易贝券"]:
                    print(f"买家由{reserve_fund_bing_sales}邀请进来，"
                    ...
                    print(f"出钱方由{reserve_fund_bing_sales}邀请进来，"
                          f"该{reserve_fund_bing_sales}的id是：{bind_buyer_relationship_data[f'{reserve_fund_bing_sales}']}，"
                ...
                if reserve_fund_bing_sales_id != None and reserve_fund_sales_bing_sale_id != None:
                    ...
                    print(f"储备池买家上级{reserve_fund_bing_sales_id}没有绑定的上级销售/业务焕商，不会获得分佣")
                else:
                    print(f"储备池买家没有绑定上级销售/业务焕商，不会获得分佣")
                ...
                print(f"二级分佣是拿{reserve_fund_agent}的钱来分，该{reserve_fund_agent}最终实际获得的分佣金额是：{first_region_commission} - "
        ...
        if self.payment_method != "易贝券":
            ...
            if bind_relationship_data != None and service_fee_second_payagent_ratio != None:
                ...
                if self.payment_method == "易贝":
                    print(f"买家是由{service_fee_bing_sales}邀请进来的，并且设置了二级分佣比例，需要走二级分佣")
                elif self.payment_method == "抵工资":
                    print(f"企业是由{service_fee_bing_sales}邀请进来的，并且设置了二级分佣比例，需要走二级分佣")
                elif self.payment_method == "家人购":
                    print(f"家人是由{service_fee_bing_sales}邀请进来的，并且设置了二级分佣比例，需要走二级分佣")
                else:
                    print(f"卖家是由{service_fee_bing_sales}邀请进来的，并且设置了二级分佣比例，需要走二级分佣")
                ...
                print(f"二级分佣是拿{service_fee_agent}的钱来分，该{service_fee_agent}最终实际获得的分佣金额是：{first_region_commission} - "
