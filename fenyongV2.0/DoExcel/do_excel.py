...
class DoExcel:
        ...
        for key in mode:  # 遍历这个存在配置文件里面的字典 也就是表单名
            ...
            if mode[key]["焕商分佣"] == 'all':  # 判断value
                for i in range(2, sheet.max_row + 1):
                    ...
                    row_data['bind_relationship_data']=sheet.cell(i,13).value
                    row_data['second_payagent_ratio'] = sheet.cell(i, 14).value
            ...
            # 跑指定用例
                ...
                for i in mode[key]["焕商分佣"]:  # case_id
                    ...
                    row_data['bind_relationship_data'] = sheet.cell(i+1, 13).value
                    row_data['second_payagent_ratio'] = sheet.cell(i+1, 14).value
                    row_data['sheet_name'] = key
                    row_data['surroundings'] = mode[key]["环境"]
                    test_data.append(row_data)
    ...
    def write_back(file_name, sheet_name, i,actual_userid, actual_changes,
        ...
        sheet.cell(i, 15).value = actual_userid  # 实际
    ...
    def bing_sale_id(cls, file_name, sheet_name, case_id, shangji_sale_id):
        ...
        wb = load_workbook(file_name)
        sheet = wb[sheet_name]
        sheet.cell(case_id + 1, 13).value = shangji_sale_id  # 计算
        wb.save(file_name)
    ...
    def second_payagent_ratio(self, file_name, sheet_name, case_id, bili):
        ...
        wb = load_workbook(file_name)
        sheet = wb[sheet_name]
        sheet.cell(case_id + 1, 14).value = bili  # 计算
        wb.save(file_name)
