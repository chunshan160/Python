...
class DoExcel:
    ...
    def get_data(cls, file_name):
        ...
        test_data = []
        for key in config:  # 遍历这个存在配置文件里面的字典 也就是表单名
            sheet_name=config[key]['sheet_name']
            sheet = wb[sheet_name]  # 打开Excel里的这个表单
            # 跑全部用例
            if config[key]["TestCase"] == 'all':  # 判断value
                for i in range(2, sheet.max_row + 1):
                    ...
                    row_data['goodsname'] = sheet.cell(i, 7).value
                    ...
                    row_data['ip'] = config[key]["IP"]
                    row_data['surroundings'] = config[key]["surroundings"]
                    row_data['payPassword']=config[key]["payPassword"]
                    test_data.append(row_data)
            # 跑指定用例
            else:
                for i in config[key]["TestCase"]:  # case_id
                    row_data = {}
...
if __name__ == '__main__':
    a= DoExcel().get_data(test_data_path)
    print(a)
