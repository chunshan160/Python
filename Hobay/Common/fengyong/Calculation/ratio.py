...
def ratio(ip, user_id):
    ...
    if user_special_ratio_data != None:
        ...
        my_logger.info(f"设置的临时采购易贝费率是：{user_special_buy_cbp_ratio}，现金费率是：{user_special_buy_cash_ratio}，设置的临时销售现金费率是：{user_special_sale_cash_ratio}")
    ...
    if platform_promotion_ratio_data != None:
        ...
        my_logger.info(f"设置的活动采购易贝费率是：{platform_promotion_buy_cbp_ratio}，现金费率是：{platform_promotion_buy_cash_ratio}，销售现金费率是：{platform_promotion_sale_cash_ratio}")
    ...
    # 只设置活动费率
        ...
        # 活动现金费率
        if user_special_buy_cash_ratio != None:
            my_logger.info("设置了活动现金费率，采购现金费率按照后台设置的活动现金费率")
            buy_cbp_ratio = user_special_buy_cbp_ratio
        else:
            buy_cbp_ratio = user_grade_buy_cbp_ratio
            my_logger.info("没有设置活动现金费率，采购现金费率按照会员等级")

        # 销售现金费率
        if user_special_sale_cash_ratio != None:
            my_logger.info("设置了活动现金费率，销售现金费率按照后台设置的活动现金费率")
            buy_cbp_ratio = user_special_buy_cbp_ratio
        else:
            buy_cbp_ratio = user_grade_buy_cbp_ratio
            my_logger.info("没有设置活动现金费率，销售现金费率按照会员等级")
    ...
    my_logger.info(f"最终使用的采购易贝费率是：{float(buy_cbp_ratio)}，采购现金费率是：{float(buy_cash_ratio)}，销售现金费率是：{float(sale_cash_ratio)}")
