...
from decimal import *
from Common.fengyong.tools.new_round import new_round
from Common.fengyong.tools.paixu import paixu
...
class Dividend:
    ...
    def recharge(self, recharge, A=0.60):
        ...
        my_logger.info("充值服务费金额是：%.2f" % (recharge))
        my_logger.info("存入储备池金额是：%.2f * %.2f = %.2f" % (recharge, A, M))
        my_logger.info("区代理商获得的现金交易分佣是：%.2f * %.2f = %.2f" % (self.buyer_area_proportion, (recharge - M), N2))
        my_logger.info("市代理商获得的现金交易分佣是：%.2f * %.2f = %.2f" % (
            (self.buyer_city_proportion - self.buyer_area_proportion), (recharge - M), N3))
        my_logger.info("省代理商获得的现金交易分佣是：%.2f * %.2f = %.2f" % (
            (self.buyer_province_proportion - self.buyer_city_proportion), (recharge - M), N4))
        my_logger.info("平台所得充值分佣金额是：%.2f - %.2f - %.2f - %.2f = %.2f" % ((recharge - M), N2, N3, N4, N5))
        return M
    ...
    def pay_service_fee(self, payment_method, service_fee):
        ...
        if self.buyer_personal_proportion == None:
            ...
            my_logger.info("个人焕商获得得的服务费分佣是：{0} * {1} = {2}".format(service_fee, self.buyer_personal_proportion, M1))
        ...
        if self.buyer_province_proportion == None and self.buyer_city_proportion == None and self.buyer_area_proportion == None:
            ...
            if self.buyer_personal_proportion != None and self.buyer_personal_proportion > min_proportion:
                ...
                if self.buyer_area_proportion == None:
                    ...
                    my_logger.info("区代理商获得的服务费分佣是：{0} * {1} = {2}".format(self.buyer_area_proportion, (service_fee - M1), M2))
                ...
                if self.buyer_city_proportion == None:
                    ...
                    my_logger.info("市代理商获得的服务费分佣是：{0} * {1} = {2}".format(new_round(float(a), 2), (service_fee - M1), M3))
                ...
                if self.buyer_province_proportion == None:
                    ...
                    my_logger.info("省代理商获得的服务费分佣是：{0} * {1} = {2}".format(new_round(float(a), 2), (service_fee - M1), M4))

            else:
                my_logger.info("使用算法一")
                ...
                if self.buyer_area_proportion == None:
                    ...
                    M2 = new_round(float(M2), 2)
                    M2 = Decimal(str(M2)).quantize(Decimal('0.00'))
                    my_logger.info("区代理商获得的服务费分佣是：{0} * {1} = {2}".format(new_round(float(a), 2), service_fee, M2))
                ...
                if self.buyer_city_proportion == None:
                    ...
                    M3 = new_round(float(M3), 2)
                    M3 = Decimal(str(M3)).quantize(Decimal('0.00'))
                    my_logger.info("市代理商获得的服务费分佣是：{0} * {1} = {2}".format(new_round(float(a), 2), service_fee, M3))
                ...
                if self.buyer_province_proportion == None:
                    ...
                    M4 = new_round(float(M4), 2)
                    M4 = Decimal(str(M4)).quantize(Decimal('0.00'))
                    my_logger.info("省代理商获得的服务费分佣是：{0} * {1} = {2}".format(new_round(float(a), 2), service_fee, M4))

        M5 = service_fee - M1 - M2 - M3 - M4  # 平台所得分佣
        M5 = new_round(float(M5), 2)
        M5 = Decimal(str(M5)).quantize(Decimal('0.00'))
        my_logger.info("平台获得的服务费分佣是：{0} - {1} - {2} - {3} - {4} = {5}".format(service_fee, M1, M2, M3, M4, M5))
    ...
    def cash_service_fee(self, charge_amount, reserve_pool):
        ...
        my_logger.info("储备池金额是：%.2f" % (reserve_pool))
        my_logger.info("----------开始计算储备池分佣----------")
        my_logger.info(f"买家上级省代理商分佣比例是：{self.buyer_province_proportion}，买家上级市代理商分佣比例是：{self.buyer_city_proportion}，"
        ...
        M1 = new_round(float(M1), 2)
        M1 = Decimal(str(M1)).quantize(Decimal('0.00'))
        my_logger.info("个人焕商获得得的现金交易分佣是：{0} * {1} = {2}".format(charge_amount, self.buyer_personal_proportion, M1))
        ...
        if self.buyer_province_proportion == None and self.buyer_city_proportion == None and self.buyer_area_proportion:
            ...
            if reserve_pool > M1:
                ...
                if self.buyer_area_proportion == None:
                    M2 = 0
                    my_logger.info("该买家没有上级区代理商，不会获得分佣。")
                else:
                    a = self.buyer_area_proportion * (reserve_pool - M1)  # 区代理商所得分佣
                    M2 = new_round(float(a), 2)
                    M2 = Decimal(str(M2)).quantize(Decimal('0.00'))
                    my_logger.info("区代理商获得的现金交易分佣是：{0} * {1} = {2}".format(self.buyer_area_proportion, (reserve_pool - M1), M2))

                if self.buyer_city_proportion == None:
                    M3 = 0
                    my_logger.info("该买家没有上级市代理商，不会获得分佣。")
                else:
                    if self.buyer_area_proportion == None:
                        self.buyer_area_proportion = 0
                        a = self.buyer_city_proportion
                    ...
                    M3 = new_round(float(M3), 2)
                    M3 = Decimal(str(M3)).quantize(Decimal('0.00'))
                    my_logger.info("市代理商获得的现金交易分佣是：{0} * {1} = {2}".format(a, (reserve_pool - M1), M3))

                if self.buyer_province_proportion == None:
                    M4 = 0
                    my_logger.info("该买家没有上级省代理商，不会获得分佣。")
                else:
                    if self.buyer_city_proportion == None:
                        self.buyer_city_proportion = 0
                        if self.buyer_area_proportion == None:  # 市代为空 区代也为空
                    ...
                    M4 = new_round(float(M4), 2)
                    M4 = Decimal(str(M4)).quantize(Decimal('0.00'))
                    my_logger.info("省代理商获得的现金交易分佣是：{0} * {1} = {2}".format(a, (reserve_pool - M1), M4))

                M5 = reserve_pool - M1 - M2 - M3 - M4  # 平台所得分佣
                M5 = new_round(float(M5), 2)
                M5 = Decimal(str(M5)).quantize(Decimal('0.00'))
                my_logger.info("平台获得的现金交易分佣是：{0} - {1} - {2} - {3} - {4} = {5}".format(
                ...
                my_logger.info(f"储备池金额小于等于个人焕商应得金额，所以储备金全部分给个人焕商，个人焕商获得的储备池分佣是：{reserve_pool}")
                my_logger.info("省市区平台获得的储备池分佣全部为0")
