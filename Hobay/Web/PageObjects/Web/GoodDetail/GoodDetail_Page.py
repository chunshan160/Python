...
from Web.PageLocators.Web.GoodDetail import GoodDetail as GD
...
class GoodDetailPage(BasePage):
    ...
    # 购买数量
    def add_amount(self, amount, text=""):
        doc = text + "点击【购买数量】输入框-"
        time.sleep(0.5)
        UserLog().info("输入的商品数量是:" + amount)
        self.input_text(GD.add_amount, amount, doc=doc)
    ...
    # 购买商品
    def buy_good(self, amount=1, text=""):
        doc = text + "购买商品-默认一件-"
        self.buy_now(text=doc)
        if amount != 1:
            self.add_amount(amount, text=doc)
        else:
            self.add(text=doc)
