...
from PageLocators.H5.Login import PwdLogin
from PageLocators.H5.Login import PasswordLogin as PL
# from PageLocators.Web.za.Login import PwdLogin
# from PageLocators.Web.za.Login import Retrieve_Password as RP
import time
...
class LoginPage:
    ...
    def login(self, username, password):
        # 输入手机号
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located((PwdLogin.name))).send_keys(username)
        # 点击下一步
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located((PwdLogin.next))).click()
        # 输入密码
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located((PL.enter_password))).send_keys(password)
        # 点击登录
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located((PL.login))).click()
        # 首页弹出来的城市定位
    ...
    #     # 输入手机号
    #     WebDriverWait(self.driver, 20).until(EC.visibility_of_element_located((PwdLogin.name))).send_keys(username)
    #     # 点击下一步
    #     WebDriverWait(self.driver, 20).until(EC.visibility_of_element_located((PwdLogin.next))).click()
    #     # 输入密码
