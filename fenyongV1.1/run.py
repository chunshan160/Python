...
from TestCases.test_buygoods3 import TestBuyGoods
import unittest
...
with open(test_report_path, 'wb') as file:
    # 执行用例
    runner = HTMLTestRunnerNew.HTMLTestRunner(stream=file, verbosity=2, title="自动化测试报告", description="V2.5.9焕商分佣1.0",
                                              tester="春衫")
    runner.run(suite)
